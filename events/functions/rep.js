...
module.exports = async (message, client) => {
	...
	if (message.channel.id === tradeRepChannel) {
		...
		if (!userData) {
			...
			userData.TradeRep += 1;
			userData.save();
			message.channel.send(`${message.mentions.members.first()}, kamu dapet +1 Trade Rep!`)
		...
		if (!userData) {
			...
			newData.save();
			message.reply(`${message.mentions.members.first()}, kamu dapet +1 General Rep!`)

		} else if (userData) {

			userData.GeneralRep += 1;
			userData.save();
			message.channel.send(`${message.mentions.members.first()}, kamu dapet +1 General Rep!`)
